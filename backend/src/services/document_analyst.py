...
class DocumentAnalyst:
    ...
    def _assess_quality(self, result: Dict[str, Any]) -> Dict[str, float]:
        ...
        if chunks:
            # Handle chunks as dictionaries with 'content' field
            chunk_sizes = [len(c.get('content', '')) if isinstance(c, dict) else len(c) for c in chunks]
            avg_chunk_size = sum(chunk_sizes) / len(chunks)
            ideal_chunk_size = 1000
            scores["chunking"] = max(0.0, 1.0 - abs(avg_chunk_size - ideal_chunk_size) / ideal_chunk_size)
        else:
            scores["chunking"] = 0.0

        # Consciousness integration quality
        basins_created = result.get("basins_created", 0)
        thoughtseeds_generated = result.get("thoughtseeds_generated", [])
        # Handle both list and int for thoughtseeds_generated
        num_thoughtseeds = len(thoughtseeds_generated) if isinstance(thoughtseeds_generated, list) else thoughtseeds_generated
        scores["consciousness_integration"] = min(
            (basins_created + num_thoughtseeds) / (len(concepts) * 2), 1.0
