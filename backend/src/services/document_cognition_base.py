...
class DocumentCognitionBase:
    ...
    def record_successful_pattern(self, category: str, pattern: Dict[str, Any]):
        """
        Record a successful pattern for agent learning.

        Agents get smarter by remembering what works.
        """
        if category not in self.knowledge:
            self.knowledge[category] = []
        ...
        similar_found = False
        for existing_pattern in self.knowledge[category]:
            if existing_pattern.get("pattern_type") == pattern.get("pattern_type"):
                # Update success rate
                current_rate = existing_pattern.get("success_rate", 0.5)
                new_rate = (current_rate * 0.9) + (pattern.get("quality_score", 0.5) * 0.1)
                existing_pattern["success_rate"] = new_rate
                existing_pattern["last_seen"] = pattern.get("timestamp")
                similar_found = True
                break

        if not similar_found:
            # Add new pattern
            self.knowledge[category].append(pattern)

        self.save_knowledge()

    def boost_strategy_priority(self, category: str, strategy_name: str, boost_amount: float):
        """
        Boost priority of a strategy that appears frequently.

        Example: If "THEORETICALLY_EXTENDS" relationship appears often,
                 boost its priority so LLM looks for it more.
        """
        if category not in self.knowledge:
            self.knowledge[category] = []

        # Find existing strategy
        for strategy in self.knowledge[category]:
            if strategy.get("name") == strategy_name:
                current_priority = strategy.get("priority", 0.5)
                strategy["priority"] = min(1.0, current_priority + boost_amount)
                self.save_knowledge()
                return
